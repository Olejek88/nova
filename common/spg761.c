...
static int  spg761_date(struct device *, char *, char **);
static int  spg761_set_date(struct device *, char *, char **);
static int  spg761_get_date(struct device *, char **);

static int  spg761_get_string_param(struct device *dev, const char *addr, char **save, uint8_t type, uint16_t padr);
static int  spg761_set_string_param(struct device *dev, const char *addr, char *load, uint8_t type, uint16_t padr);
...
static int  spg761_date(struct device *dev, char *ret, char **save)
{
	int	rt=0;
	if (strlen (ret)==0)
		rt = spg761_get_string_param(dev, "60", save, TYPE_CONST, 560);
	else
		rt = spg761_set_string_param(dev, "60", ret, TYPE_CONST, 560);
	return	rt;
}

static int  spg761_get_date(struct device *dev, char **save)
{
	int	rt=0;
	rt = spg761_get_string_param(dev, "60", save, TYPE_CONST, 560);
	return	rt;
...
static int  spg761_set_date(struct device *dev, char *ret, char **save)
{
	int	rt=0;
	rt = spg761_set_string_param(dev, "60", ret, TYPE_CONST, 560);
	return	rt;
...
spg761_get_string_param(struct device *dev, const char *addr, char **save, uint8_t type, uint16_t padr)
{
	...
	if (type==TYPE_CONST) {
		...
		if (atoi(addr)==60 || atoi(addr)==20) {
			...
			sscanf(value_string,"%d-%d-%d/%d:%d:%d",&time_from->tm_mday, &time_from->tm_mon, &time_from->tm_year, &time_from->tm_hour, &time_from->tm_min, &time_from->tm_sec);
			sprintf(value_string,"%d-%02d-%02d,%02d:%02d:%02d",time_from->tm_year+2000, time_from->tm_mon, time_from->tm_mday, time_from->tm_hour, time_from->tm_min, time_from->tm_sec);
			*save=value_string;
		}
		else	{
			value_string=malloc (100);
			 ...
			 for (np=0; np<ARRAY_SIZE(archive_hour761); np++)
				{
				 ...
				 if (tim->tm_mon>12 || tim->tm_year>2100 || tim->tm_mday>31)
    					snprintf(ptr->datetime, sizeof(ptr->datetime),"%d-%02d-%02d %02d:%02d:%02d",2000, 1, 1, 0, 0, 0);
    				 else
    				 ...
    				 //printf ("[%d/%d][%s] %f\n",archive_hour761[np].id,ptr->num,ptr->datetime,ptr->params[archive_hour761[np].id]);
    				}
    			 if (type==TYPE_DAYS)
			 ...
			 //if (archive_month761[np].no==no)
				{
				 sscanf(alog->time[archive_month761[np].id][i],"%d-%d-%d/%d:%d:%d",&tim->tm_mday, &tim->tm_mon, &tim->tm_year, &tim->tm_hour, &tim->tm_min, &tim->tm_sec);
    				 snprintf(ptr->datetime, sizeof(ptr->datetime),"%d-%02d-%02d 00:00:00",tim->tm_year+2000, tim->tm_mon, tim->tm_mday);
...
spg761_h_archiv(struct device *dev, int no, const char *from, const char *to, struct archive **save)
{
	...
	while (sttime<fntime)
		...
		time_from->tm_mon++;
		time_to->tm_mon++;
		printf ("%d-%02d-%02d,%02d:%02d:%02d - %d-%02d-%02d,%02d:%02d:%02d [%ld-%ld-%ld]\n",time_from->tm_year, time_from->tm_mon, time_from->tm_mday, time_from->tm_hour, time_from->tm_min, time_from->tm_sec, time_to->tm_year, time_to->tm_mon, time_to->tm_mday, time_to->tm_hour, time_to->tm_min, time_to->tm_sec,sttime,entime,fntime);
			...
			try=5;
	    		while (try--)
			    {
			     ...
			     if (-1 == analyse_sequence (dev->buf.p, dev->buf.len, answer, ANALYSE, archive_hour761[i].id, &alog))	continue;
			     break;
		    	    }
		    }
		 ret = make_archive(dev, no, &alog, TYPE_HOURS, save, recs);
		 recs+=alog.quant_param;
		 sttime+=3600*25;
		 ret=0;
	    }
	if (recs == 0)  {
			...
			try=5;
	        	while (try--)
				{
				 ...
				 if (-1 == generate_sequence (dev, TYPE_DAYS, FNC_time_array, addr, 0, 1, no, time_from, time_to, ident_query)) 	break;
	 			 ...
	 			 ctx->send_query_sz = strlen(ident_query);
				 if (-1 == dev_query(dev))   continue;
				 if (-1 == analyse_sequence (dev->buf.p, dev->buf.len, answer, ANALYSE, archive_day761[i].id, &alog))		continue;
				 break;
				 }
			}
    		 ret = make_archive(dev, no, &alog, TYPE_DAYS, save, recs);
    		 recs+=alog.quant_param;
		 sttime+=3600*24*15;
	    	 ret=0;
		}
	...
	while (sttime<fntime)
		{
		 ...
		 time_to->tm_mon++;
		 if (!dev->quiet)
			    fprintf(stderr, "%d-%02d-%02d,%02d:%02d:%02d - %d-%02d-%02d,%02d:%02d:%02d [%ld-%ld-%ld]\n",time_from->tm_year, time_from->tm_mon, time_from->tm_mday, time_from->tm_hour, time_from->tm_min, time_from->tm_sec, time_to->tm_year, time_to->tm_mon, time_to->tm_mday, time_to->tm_hour, time_to->tm_min, time_to->tm_sec,sttime,entime,fntime);
    		 ...
    		 for (i=0; i<ARRAY_SIZE(archive_month761); i++)	{
	        	...
	        	while (try--)
				{
				 ...
				 if (-1 == generate_sequence (dev, TYPE_MONTH,FNC_time_array, addr, 0, 1, no, time_from, time_to, ident_query)) 	break;
	 			 ...
	 			 ctx->send_query_sz = strlen(ident_query);
				 if (-1 == dev_query(dev))   continue;
				 if (-1 == analyse_sequence (dev->buf.p, dev->buf.len, answer, ANALYSE, archive_month761[i].id, &alog))		continue;
				 break;
	    			}
			}
		 ret = make_archive(dev, no, &alog, TYPE_MONTH, save, recs);
		 recs+=alog.quant_param;
		 sttime+=3600*24*365*2;
	    	 ret=0;
		}
	if (recs == 0)  {
		*save = alloc_archive();
...
spg761_events(struct device *dev, const char *from, const char *to, struct events **save)
{
	...
	if (0 == checkdate (TYPE_EVENTS,ret_fr,ret_to,time_from,time_to,time_cur,&tim,&sttime,&fntime))
		{
		 localtime_r(&sttime,time_from);
		 ...
		 time_to->tm_mon++;
		 if (!dev->quiet)
			    fprintf(stderr, "%d-%02d-%02d,%02d:%02d:%02d - %d-%02d-%02d,%02d:%02d:%02d [%ld-%ld]\n",time_from->tm_year, time_from->tm_mon, time_from->tm_mday, time_from->tm_hour, time_from->tm_min, 0, time_to->tm_year, time_to->tm_mon, time_to->tm_mday, time_to->tm_hour, 0, 0,sttime,fntime);
		 if (-1 == generate_sequence (dev, TYPE_EVENTS, FNC_time_array, "98", 1, 0, 1, time_from, time_to, ident_query)) ret = -1;
		 //fprintf(stderr, "generate_sequence [%d]\n",ret);
 		 ctx->send_query = ident_query;
 		 ctx->send_query_sz = strlen(ident_query);
		 if (0 == ret && -1 == dev_query(dev)) ret = -1;
		 //fprintf(stderr, "dev_query [%d] (%d)\n",ret,ctx->send_query_sz);
		 if (0 == ret && -1 == analyse_sequence (dev->buf.p, dev->buf.len, answer, ANALYSE, 0, &alog)) ret = -1;
		 //fprintf(stderr, "analyse_sequence [%d] q=%d\n",ret,alog.quant_param);
		 ...
		 //fprintf(stderr, "alog.quant_param q=%d\n",alog.quant_param);
    ...
    switch (func)
	{
	 ...
	 case FNC_time_array:
		 sprintf (buffer,"%c%c%c%c%c%c%c%c%c%c%c%c%c%c%s%c%c%d%c%d%c%d%c%d%c%d%c%d%c%c%d%c%d%c%d%c%d%c%d%c%d%c%c%c",DLE,SOH,dev->devaddr,0x80,DLE,ISI,func,0x53,0x30,DLE,STX,HT,npipe+48,HT,padr,FF,
			  HT,to->tm_mday,HT,to->tm_mon,HT,to->tm_year,HT,to->tm_hour,HT,0,HT,0,FF,HT,from->tm_mday,HT,from->tm_mon,HT,from->tm_year,HT,from->tm_hour,HT,0,HT,0,FF,DLE,ETX);
	 	 ks = calc_bcc ((uint8_t*)buffer+2, strlen(buffer)-2);
		 sprintf (sequence,"%s%c%c",buffer,(uint8_t)(ks>>8),(uint8_t)(ks%256));
